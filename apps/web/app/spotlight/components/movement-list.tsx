...
export function MovementList({ movements, onSelect }: {
  ...
  return <div className={styles.movementList}>
    {movements.map((movement) => (
      <button className={styles.movementCard} type="button" key={movement.id} onClick={() => onSelect(movement)} aria-label={`View ${movement.aircraft.type} ${movement.aircraft.registration ?? "registration pending"} details`}>
        <div className={styles.timeBlock}>
          <strong>{formatTime(movement.estimatedTime)}</strong>
          <MovementTypeIndicator movementType={movement.movementType} compact />
        </div>
        <div className={styles.aircraftBlock}><strong>{movement.aircraft.type}</strong><span className={movement.aircraft.registration ? "" : styles.pendingRegistration}>{movement.aircraft.registration ?? "Registration not assigned yet"}</span></div>
